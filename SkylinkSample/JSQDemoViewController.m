...
- (IBAction)titleButtonTapped:(id)sender {
    ...
    self.navigationItem.hidesBackButton = self.isMinimized = !self.isMinimized;
                      ...
                      date:(NSDate *)date
{
    ...
    if (text.length > 0) {
        ...
        [self.originalMessageArray addObject:@{@"user": sender, @"text": text}];
        [((TEMAppDelegate*)[UIApplication sharedApplication].delegate).roomViewController sendChatMessage:text target:self.peerId];
...
{
    [self scrollToBottomAnimated:YES];
    JSQMessage *copyMessage = [JSQMessage messageWithText:message sender:nick];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [JSQSystemSoundPlayer jsq_playMessageReceivedSound];
        [self.messages addObject:copyMessage];
        [self.originalMessageArray addObject:@{@"user": nick, @"text": message}];
        [self finishReceivingMessage];
    });
}
...
- (void)setupModel:(NSMutableArray*)messageArray
{
     ...
     *  You should have a mutable array or orderedSet, or something.
     */
    self.originalMessageArray = messageArray;
    self.messages = [[NSMutableArray alloc] init];
    for (NSDictionary *dict in messageArray)
        [self.messages addObject:[JSQMessage messageWithText:[dict objectForKey:@"text"] sender:[dict objectForKey:@"user"]]];
...
- (void)keyboardWillShow:(NSNotification*)notification
{
    ...
    ((UIButton*)self.navigationItem.titleView).enabled = FALSE;
}

- (void)keyboardWillHide:(NSNotification*)notification
{
    self.navigationItem.hidesBackButton = FALSE;
    ((UIButton*)self.navigationItem.titleView).enabled = TRUE;
}

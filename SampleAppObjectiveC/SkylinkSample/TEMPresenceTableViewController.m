...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL openChat = !(((TEMAppDelegate*)[UIApplication sharedApplication].delegate).appConfig.fileTransfer && ((TEMAppDelegate*)[UIApplication sharedApplication].delegate).appConfig.dataChannel);
...
- (void)mediaPicker:(MPMediaPickerController *)mediaPicker didPickMediaItems:(MPMediaItemCollection *)mediaItemCollection
{
    ...
    [((TEMAppDelegate*)[UIApplication sharedApplication].delegate).roomViewController startFileTransfer:peerId url:[mediaItemCollection.representativeItem valueForProperty:MPMediaItemPropertyAssetURL] type:SKYLINKAssetTypeMusic];
}
        ...
        case 1:
            if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]) {
                ...
                if (((TEMAppDelegate*)[UIApplication sharedApplication].delegate).isPad) {
                    ...
                    [pickerController setPreferredContentSize:CGSizeMake(320, 480)];
                    [(TEMAppDelegate*)[UIApplication sharedApplication].delegate  presentOverRoomViewController:pickerController];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if (selectedRow > 0)
        peerId = ((PresenceTableEntry*)[presenceArray objectAtIndex:selectedRow]).peerId;
    [((TEMAppDelegate*)[UIApplication sharedApplication].delegate).roomViewController startFileTransfer:peerId url:info[UIImagePickerControllerReferenceURL] type:SKYLINKAssetTypePhoto];
}
...
- (void)filePicker:(TEMFilePickerController*)filePicker didPickFile:(NSURL*)fileURL
{
    ...
    if (selectedRow > 0)
        peerId = ((PresenceTableEntry*)[presenceArray objectAtIndex:selectedRow]).peerId;
    [((TEMAppDelegate*)[UIApplication sharedApplication].delegate).roomViewController startFileTransfer:peerId url:fileURL type:SKYLINKAssetTypeFile];
}
...
- (IBAction)titleButtonTapped:(id)sender {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:TEM_MINIMIZE_PRESENCE object:nil];
}
